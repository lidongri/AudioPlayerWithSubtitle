//
//  KJCircleSliderView.m
//  BlueWhaleProject
//
//  Created by 李雪健 on 2021/12/22.
//

#import "KJCircleSliderView.h"

@interface KJCircleSliderView ()
...
@implementation KJCircleSliderView
...
- (void)drawRect:(CGRect)rect {
     ...
     * 这里用了另外一个比较投机的方法，先算出亮点连线在y轴上投影的长度，然后根据移动点在y轴上相对于圆心的位置将这个绝对长度a和圆心y坐标相加减。
     */
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    double touchDist = [KJCircleSliderView distanceBetweenPointA:starTouchPoint pointB:self.lastPoint];
    if (touchDist > 44) {
        self.interaction = NO;
    ...
    //    double dist = [KJCircleSliderView distanceBetweenPointA:starTouchPoint pointB:self.drawCenter];
...
- (void)moveHandlerWithPoint:(CGPoint)point {
    ...
    CGFloat angle = [KJCircleSliderView calculateAngleWithRadius:self.radius
...
+ (CGFloat)calculateAngleWithRadius:(CGFloat)radius
    ...
    //a^2 = b^2 + c^2 - 2bccosA;
    CGFloat cosA = (2 * radius * radius - powf([KJCircleSliderView distanceBetweenPointA:startCenter pointB:endCenter], 2)) / (2 * radius * radius);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    KJCircleSliderView *circleSlider = (KJCircleSliderView *)object;
