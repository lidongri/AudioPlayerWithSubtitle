...
{
    [self setShadowWithColor:color offSet:offSet opacity:opacity radius:radius];
...
-(void)setInnerShadowWithRect:(CGRect)rect shadowColor:(UIColor *)color offSet:(CGSize)offSet opacity:(float)opacity radius:(CGFloat)radius
{
    ...
    [shadowLayer setShadowOffset:offSet];
    ...
    CGPathRef someInnerPath = [UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:radius].CGPath;
...
-(void)addGradientWithColors:(NSArray*)colors rect:(CGRect)rect gradientType:(GradientDirection)type
{
    ...
    gradientLayer.endPoint = type==GradientTypeLeftToRight ? CGPointMake(1.0, 0) : CGPointMake(0, 1.0);
    NSMutableArray *list = [NSMutableArray array];
    for (UIColor *color in colors) {
        [list addObject:(__bridge id)color.CGColor];
    }
